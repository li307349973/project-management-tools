...
```bash
cd /Users/mac/.codex/skills/work-hour-audit
...
- 优先使用 JIRA Token 直连 REST API；Token 可来自环境变量或 macOS Keychain；未配置 Token 时，回退到 Chrome 已登录的 JIRA 页面
- 输出 `jira_verify.json`

Token 直连方式（任选一种）：

1. 从 macOS Keychain 读取（推荐）：

```bash
read -s JIRA_TOKEN
security add-generic-password -U -a liqing -s codex-work-hour-audit-jira-token -w "$JIRA_TOKEN"
unset JIRA_TOKEN
node scripts/verify_jira.js
```

默认读取：
- Keychain service: `codex-work-hour-audit-jira-token`
- Keychain account: `liqing`
...
```bash
export JIRA_TOKEN='你的JiraToken'
node scripts/verify_jira.js
```

也支持：
- `JIRA_AUTH_HEADER='Bearer xxx'`
- `JIRA_BEARER_TOKEN='xxx'`
- `JIRA_USERNAME='用户名' JIRA_API_TOKEN='xxx'`
- `JIRA_BASE_URL='http://jira.fingard.com:6001'` 覆盖默认地址
...
- Chrome 需登录轻舟平台 (`proj.fingard.com:1888`)
- JIRA 核验优先使用 Token；未配置 Token 时，Chrome 需登录 JIRA (`jira.fingard.com:6001`)
